...
void live()
{
  ...
  // output sound
  //to remove the noise just in case
  int theAdjuster = 8;

  //get analog value of the button keys
  int pinVal = analogRead(NOTE_IN_PIN);

  //evaluate the pinVal is greater than noise and button is pressed
...
void startUpTimer()
{
  //assign the current time (millis()).
  startTime = millis();
  //reset timePassed
  timePassed = 0;
  //set to true; indicate the button state is active
...
void updateTimer()
{
  //update the timer | current timer minus the last update.
...
void playCurrentNote()
{
  ...
  //hold the tone for the notes array | set the current tone to active frequency
...
void updateArraysWithNoteAndTimings()
{
  ...
  //set button state to false | not pressed
...
{
  //integer place holder for photocell's value
  int photoVal = analogRead(PHOTO_PIN);

  //returning the scaled photocell's value
...
int getRunningAverage()
{
  ...
  //total sum of photocell's input
  int totalSum = 0;

  //looping through sixteen times to calculate total sum
...
{
  //IMPLEMENT
  //check for it not to exceed limit size of the array
  if (countNotes < MAX_NOTES) {
    tone(BUZZER_PIN, notes[countNotes], durations[countNotes]);
    //change the light intensty according to note frequency and scaled it
    colorLED(map(notes[countNotes], 0, 1023, 0, 255));
    //delay to allow the tone to play
